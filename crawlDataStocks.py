...
def get_or_insert_session_id(connection, session_id_str):
    ...
    try:
        ...
        print(f"Error retrieving/inserting session_id for session_id {session_id_str}: {e}")
        return None
    finally:
        cursor.close()
...
def insert_into_db(connection, data):
    ...
    try:
        ...
        print(f"Error inserting data into detail_stock_price: {e}")
    finally:
        cursor.close()

# Đọc các mã chứng khoán từ tệp CSV
csv_directory = r'D:\UIT Folder\Code\Git\stock_uit.lab\allDataStocks'
os.makedirs(csv_directory, exist_ok=True)
stock_data = pd.read_csv(r'D:\UIT Folder\Code\Git\stock_uit.lab\AllNameStock.csv')
        ...
        for page_number in range(1, 290):
            ...
            if data:
                ...
                if 'Data' in json_data and 'Data' in json_data['Data']:
                    ...
                    print(f"No data found on page {page_number} for {symbol}")
            else:
                print(f"No response from server on page {page_number} for {symbol}")
